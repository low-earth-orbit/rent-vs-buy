...
- The US "4% rule" / ~3.2% long-horizon *floor* is a **US-exceptionalism
...
- We **removed the `−0.2` AR(1) mean-reversion** (now `0`, i.e. iid). It had been added to match
  Morningstar's 30y number, but 150 years of global history shows real returns have *at least*
...
long-horizon dispersion as iid — crashes and high-inflation decades *cluster* (positive, not
negative, effective serial dependence at long horizons). So a variance-*reducing* φ<0 points
...
2. **At the app's own 3.5%-real assumption, real historical sequencing gives ~3.2%/2.5%/2.1%
   (30/40/50y)** — *lower* than the app's parametric output. So the parametric model is, if
...
3. **Block bootstrap destroys the multi-decade mean reversion** that creates the empirical
   ...
   and can fall below iid. The floor is a property of *actual realized long paths*; reshuffling
...
`safeWithdrawalRate` at the user's current confidence. That table is a *pure-portfolio* benchmark
...
- **Keep `RETURN_AUTOCORRELATION = 0`** unless you have a *separately justified, disclosed*
